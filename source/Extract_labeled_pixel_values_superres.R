...
# Add the resolution identifier to manage filenames consistently.
spatial_resolution <- "superres"

# This ensures consistent outputs for plots and analysis.
desired_label_order <- c("Inundated", "Not inundated", "Other", "Reed", "Uncertain")
...
final_attributes_raster_path <- file.path(
...
# UPDATED: Reinstating the explicit alignment step as requested.
...
# Rename the 'x' and 'y' columns to match what rasterize_attributes expects
names(combined_pixel_data_df)[1:2] <- c("x_coord", "y_coord")
...
message("Spectral indices calculated using your custom function.")
...
message("Dominant label and mixture category assigned using your custom function.")

# Forcing a specific factor order (as in the original script)
...
message("The following ", length(attributes_to_add), " attributes will be added as new raster layers:")
