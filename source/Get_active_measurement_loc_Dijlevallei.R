...
st_write(gebieden_sf, "output/Watina/watina_gebieden.geojson", driver = "GeoJSON")
...
# Save as a GeoJSON file
st_write(meetpunten_sf, "output/Watina/watina_meetpunten.geojson", driver = "GeoJSON")

# Read the shapefile of the Dijlevallei
dijle_sf <- st_read("data/Dijlevallei_SBZ_afbakening.shp")  # Replace with actual file path

# Ensure both layers have the same CRS (Belgian Lambert 72: EPSG 31370)
dijle_sf <- st_transform(dijle_sf, crs = 31370)
meetpunten_sf <- st_transform(meetpunten_sf, crs = 31370)

# Perform spatial intersection: Keep only meetpunten inside the area
meetpunten_dijle <- st_intersection(meetpunten_sf, dijle_sf)

# Save the filtered meetpunten as a new GeoJSON file
st_write(meetpunten_dijle, "output/Watina/meetpunten_Dijlevallei.geojson", driver = "GeoJSON")

# Extract unique MeetpuntWID values from meetpunten_demer
meetpunt_wids <- unique(meetpunten_dijle$MeetpuntWID)
...
if (!is.null(peilmetingen_dijle)) {
  View(peilmetingen_dijle)
} else {
...
output_dir <- "output/Watina/timeseries_plots/Dijlevallei"
if (!dir.exists(output_dir)) {
  dir.create(output_dir)
}

# Generate and save plots for each unique MeetpuntCode
for (meetpunt in unique(peilmetingen_dijle$MeetpuntCode)) {
  # Filter data for the current MeetpuntCode
  meetpunt_data <- peilmetingen_dijle %>% filter(MeetpuntCode == meetpunt)
...
meetpunten_filtered <- meetpunten_dijle %>%
  filter(MeetpuntCode %in% unique(peilmetingen_dijle$MeetpuntCode))

# Save as a new GeoJSON file
st_write(meetpunten_filtered, "output/meetpunten_Dijlevallei_actief2018.geojson", driver = "GeoJSON", delete_dsn = TRUE)
