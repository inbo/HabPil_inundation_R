...
Analyse_and_Model_Pixel_Data.R
  	Purpose:
...
Apply_jussila_model_map.R
	...
	the spatial agreement and disagreement between the ground-truth reference labels and the model's predictions.
  	...
  	- Automated Output Management: Saves the final classified inundation map; the Reference map and the Combination map
 		as compressed GeoTIFF (.tif) and  exports the high-resolution comparison plots as PNG files to the project's 
		output directory.

Synthetize_visualize.R
	Purpose:
	This script serves as the final synthesis step for the entire project. It aggregates the outputs from
	all individual analysis runs (across multiple study sites and years) to produce high-level summary
	visualizations. Its goal is to reveal overarching trends in the Jussila model's performance and
	provide a comprehensive overview of the entire dataset.

	Key Features & Analysis Performed:
	- Automated Data Aggregation: Systematically finds and loads all `_model_performance_metrics.csv`
		and `_final_pixel_attributes.csv` files generated by previous scripts.
	- Performance vs. Time: Generates plots showing how model performance metrics (Recall, Precision, F1-Score)
		vary throughout the year by plotting them against the Julian date of the satellite image.
	- Performance vs. Landscape Context: Creates scatter plots to visualize the relationship between model
		performance and the relative amount of inundated land in the reference data for each site/year combination.
	- Overall Data Composition Visualization: Produces an aggregated stacked bar plot that shows the total
		distribution of all pixels across all datasets, segmented by dominant land cover label and purity class.
	- Overall Model Accuracy Assessment: Generates a single, comprehensive confusion matrix heatmap that
		summarizes the Jussila model's performance across every pixel from all combined study sites and years.
	- Centralized Output Management: Saves all generated synthesis plots as high-resolution PNG files into a
		dedicated `output/synthesis_plots` directory for easy access and reporting.
